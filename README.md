...
压测脚本：`bench_embedding.py`（embedding 阶梯并发）、`bench_chat.py`（问答接口并发），`set -a && source .env && set +a && python3 bench_embedding.py` 复测。

**Embedding API（SiliconFlow bge-m3）阶梯加压**：
...
40 并发内线性增长，**~92 QPS 见顶**；超限后对方排队而非拒绝（无 429，延迟劣化），监控要看延迟分位数。批量接口 30 条/次吞吐 57 条/s，是单条的 15 倍——知识库导入走批量。

**问答接口 `/api/v1/chat` 并发压测**：

| 场景 | 并发 | 成功率 | 折合 QPS | 平均延迟 |
|---|---|---|---|---|
| 缓存全 miss（完整 RAG + LLM） | 30 | 30/30 | 4.8 | 2264ms |
| 缓存全 hit（仅 embedding + 缓存） | 30 | 30/30 | 24.4 | 1174ms |

瓶颈分层：Go 服务 / Qdrant(36ms) / Redis 均不构成瓶颈；**miss 路径瓶颈在 DeepSeek 生成（2.3s/次），hit 路径瓶颈在 embedding**。客服场景提问高度重复，按 70% 缓存命中率估算混合承载 ≈ 18 QPS；对照业务体量（10 万学员、晚高峰峰值 2~5 QPS）有 3~9 倍余量。扩容路径：提缓存命中率 → DeepSeek 并发额度/多 Key → 无状态水平扩展。
